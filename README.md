...
| **Nguyễn Hoàng Hải** | **Frontend UI & Repo Maintainer** | • Chịu trách nhiệm chính: Thư mục `codebase/` & `README.md`<br>• Build Prototype: Lập trình file `codebase/index.html` hiển thị giao diện đọc slide VLearn + Sidebar.<br>• Render & Tương tác: Tích hợp Mermaid.js chuyển đổi 3 dạng sơ đồ và xử lý JavaScript sự kiện click Node $\rightarrow$ cuộn Slide (Deep-link).<br>• Quản trị Repo: Dựng cấu trúc cây thư mục repo chuẩn R7, tổng hợp file `reflection/`. |
